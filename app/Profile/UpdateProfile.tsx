...
const UpdateProfile = ({ navigation }) => {
   ...
   useEffect(() => {
      if (user) {
         setData({
            ...
            isNewUser: !user?.phone ? true : false
         })
      }
   }, [user]);
   // Handle update info
   const handelUpdateUser = () => {
      ...
      if (data.phone.length > 12) {
         return ToastAndroid.show('Phone number should be less than 13 digits!', ToastAndroid.SHORT)
      }
      dispatch(updateUserProfile(data.isNewUser, user.id, data, accessToken, resetForm));
      navigation.goBack();
      console.log('User: ', data, '| isNewUser: ', data.isNewUser)
   }
   return (
